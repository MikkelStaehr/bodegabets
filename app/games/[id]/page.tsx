...
export default async function GamePage({ params }: Props) {
  ...
  let lineupStages: { id: string; race_id: string; stage_number: number; name: string; profile: string | null; profile_image_url: string | null; start_date: string; start_time_utc: string | null; distance_km: number | null; departure: string | null; arrival: string | null; profile_score: number | null; vertical_meters: number | null; climbs: import('@/types/cycling').CyclingStageClimb[]; results_uploaded_at: string | null; race_name: string; race_type: string; race_profile_image_url: string | null; cycling_block_id: string | null }[] = []
  ...
  if (typedGame.sport === 'cycling') {
    ...
    if (raceIdsForStages.length > 0) {
      const { data: stagesData } = await supabaseAdmin
        .from('cycling_stages')
        .select('id, race_id, stage_number, name, profile, profile_image_url, start_date, start_time_utc, distance_km, departure, arrival, profile_score, vertical_meters, climbs, results_uploaded_at')
